...
hook OnGameModeInit() {
	...
	ac_ACToggle[AC_DIALOGSPOOFING] = false; //Dialog
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
	...
	if(GetPlayerState(playerid) == PLAYER_STATE_ONFOOT) {
		...
		if(newkeys & KEY_CROUCH) {
			...
			if(IsPlayerInAnyVehicle(playerid) || GetPVarType(playerid, "EventToken") || (GetPVarInt(playerid,"IsInArena") && PaintBallArena[GetPVarInt(playerid, "IsInArena")][pbExploitPerm] == 1)) return 1;
