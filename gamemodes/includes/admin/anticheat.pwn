...
hook OnPlayerStateChange(playerid, newstate, oldstate) {
	new string[128];
	if(InsideMainMenu{playerid} > 0 && (newstate == PLAYER_STATE_DRIVER || newstate == PLAYER_STATE_PASSENGER)) {
        format(string, sizeof(string), "{AA3333}AdmWarning{FFFF00}: %s was auto kicked for entering a vehicle at the login screen!", GetPlayerNameEx(playerid));
        ABroadCast(COLOR_YELLOW, string, 2);
        SetTimerEx("KickEx", 1000, 0, "i", playerid);
	}
    if(InsideTut{playerid} > 0 && (newstate == PLAYER_STATE_DRIVER || newstate == PLAYER_STATE_PASSENGER)) {
        format(string, sizeof(string), "{AA3333}AdmWarning{FFFF00}: %s was auto kicked for entering a vehicle during the tutorial!", GetPlayerNameEx(playerid));
        ABroadCast(COLOR_YELLOW, string, 2);
        SetTimerEx("KickEx", 1000, 0, "i", playerid);
    }
	return 1;
...
hook OnPlayerGiveDamage(playerid, damagedid, amount, weaponid) {
    new string[128];
    if(InsideMainMenu{playerid} > 0) {
        format(string, sizeof(string), "{AA3333}AdmWarning{FFFF00}: %s was auto kicked for dealing damamge at the login screen!", GetPlayerNameEx(playerid));
        ABroadCast(COLOR_YELLOW, string, 2);
        SetTimerEx("KickEx", 1000, 0, "i", playerid);
    }
    if(InsideTut{playerid} > 0) {
        format(string, sizeof(string), "{AA3333}AdmWarning{FFFF00}: %s was auto kicked for dealing damamge during the tutorial!", GetPlayerNameEx(playerid));
        ABroadCast(COLOR_YELLOW, string, 2);
        SetTimerEx("KickEx", 1000, 0, "i", playerid);
    }
    return 0;
