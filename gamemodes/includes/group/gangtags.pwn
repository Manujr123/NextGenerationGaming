...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	...
	switch(dialogid)
	{
		...
		case DIALOG_GANGTAGS_FONT:
		{
			if(!response) return ClearAnimationsEx(playerid), TogglePlayerControllable(playerid, 1), 1;
			new gangtext[128];
			GetPVarString(playerid, PVAR_GANGTAGTEXT, gangtext, sizeof(gangtext));
			format(szMiscArray, sizeof(szMiscArray), "{AA3333}AdmWarning{FFFF00}: %s (ID: %d) (T:%d) sprayed %s", GetPlayerNameEx(playerid), playerid, GetPVarInt(playerid, PVAR_GANGTAGID), gangtext);
...
CMD:edittagpoint(playerid, params[])
{
	if(PlayerInfo[playerid][pAdmin] >= 4 || PlayerInfo[playerid][pGangModerator] == 2)
	{
		...
		else SendClientMessage(playerid, COLOR_GRAD1, "You're too far from the gang tag.");
	}
	else SendClientMessage(playerid, COLOR_GRAD1, "You are not authorized to use this command.");
	return 1;
}
...
CMD:erasetag(playerid, params[])
{
    if(PlayerInfo[playerid][pAdmin] >= 4 || PlayerInfo[playerid][pASM] >= 1 || PlayerInfo[playerid][pGangModerator] == 2)
    {
        ...
        GangTag_Save(playerid, i, "GANG", 0);
        return 1;
    }
    else SendClientMessage(playerid, COLOR_GRAD1, "You are not authorized to use this command.");
    return 1;
