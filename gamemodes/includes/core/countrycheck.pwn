...
CountryCheck(playerid)
{
	...
	new ccGetUrl[68];
	format(ccGetUrl, sizeof(ccGetUrl), "jamie.ng-gaming.com/geocheck.php?ip=%s&id=%d", PlayerInfo[playerid][pIP], PlayerInfo[playerid][pId]);
		...
		*/
		switch(resCode) {
			...
			case 3: {
				...
				SendClientMessage(playerid, COLOR_RED, "Your account is not set to log in from your current country. Make an Admin Request @ http://ng-gaming.com/forums");
			...
			case 4: {
				...
				Log("logs/geocheck.log", szMiscArray);
				SendClientMessage(playerid, COLOR_RED, "Your account is locked due to numerous logins from incorrect countries. Make an Admin Request @ http://ng-gaming.com/forums");
				SetTimerEx("KickEx", 5000, 0, "i", playerid);
			}
			default: {
